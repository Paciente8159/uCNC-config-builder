...
var controller = app.controller('uCNCcontroller', ['$scope', '$rootScope', function ($scope, $rootScope) {
	...
	$scope.PINS = [
		...
		{ pin: 29, mcu: 'MCU_SAMD21,MCU_LPC176X,MCU_ESP8266,MCU_ESP32,MCU_RP2040' },
...
ready(function () {
    ...
    // const zipUrl1 = document.getElementById('zipUrl1').value;
    // const zipUrl2 = document.getElementById('zipUrl2').value;

    // if (!zipUrl1 || !zipUrl2) {
    //     alert('Please enter two zip file URLs.');
    //     return;
    // }
    ...
    try {
        // Fetch and load the first zip file
        const zip1 = await loadZipFromUrl(scope.getSrc(scope.VERSION));

        // Fetch and load the second zip file
        const zip2 = await loadZipFromUrl(scope.getModules(scope.VERSION));

        // Merge the contents of zip2 into zip1
        await mergeZips(zip1, zip2);

        // Generate the combined zip file and trigger download
        const newZipBlob = await zip1.generateAsync({ type: 'blob' });
        const downloadLink = document.createElement('a');
        downloadLink.href = URL.createObjectURL(newZipBlob);
        downloadLink.download = 'combined.zip';
        downloadLink.click();
    } catch (error) {
